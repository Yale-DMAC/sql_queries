...
AND bib_text.title not like '%(ILL)%'
# these are not currently excluded from report but should be
-- AND lower(bib_text.title) not like '%course read%'
-- AND lower(bib_text.title) not like '%course material%'
-- AND lower(bib_text.title) not like '%course packet%'
-- AND lower(bib_text.title) not like '%course reserve%'
-- AND lower(bib_text.title) not like '%problem set%'
-- AND lower(bib_text.title) not like '%reading packet%'
-- AND lower(bib_text.title) not like '%reading binder%'
-- AND lower(bib_text.title) not like '%readings binder%'
-- AND bib_text.title not like '%***INTERLIBRARY LOAN***%'
# excludes things already A or M
